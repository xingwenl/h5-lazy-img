 ...
 */
function load(img,center) {
	return new Promise(function(resolve, errfn) {
		var image_url = img.getAttribute("data-lazyImg");
		if(!image_url) return;
		//本地缓存路径
		var hb_path = '_downloads/image/' + md5(image_url) + '.jpg'; //HBuilder平台路径
		var sd_path = ""
		if (mui.os.plus) {
			sd_path = plus.io.convertLocalFileSystemURL(hb_path); //SD卡绝对路径
		}
		var obj = {
			image_url: image_url,
			hb_path: hb_path,
...
//			console.log('已存在,直接显示==' + sd_path);
			resolve(temp)
			img.setAttribute("src", sd_path)
			img.setAttribute("data-load", "true")
			if (center) {//设置图片居中显示
				img.setAttribute("style",setImgStyle(temp))
			}
			img.onload = null
			return false
...
function setLoaded(obj, bg_url) {
	...
	obj.setAttribute("data-preview-src", bg_url);
